...
import type { BenchmarkAgentSubmission, BenchmarkCase, BenchmarkMatrixCell } from "./types.ts";

describe("computeSubmissionProgressCounts", () => {
  it("PS-01: derives matrix totals from submissions without double counting", () => {
    const matrix: BenchmarkMatrixCell[] = [{ agentFramework: "zeval", model: "m1", enabled: true, timeoutMs: 1000, maxTurns: 1, concurrency: 1 }];
    const cases: BenchmarkCase[] = [{ caseId: "c1", taskId: "t", input: {}, expected: {} }, { caseId: "c2", taskId: "t", input: {}, expected: {} }];
