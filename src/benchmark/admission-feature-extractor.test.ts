...
describe("extractAdmissionFeatures", () => {
  ...
  it("maps capabilities into the fixed human-review channel taxonomy", () => {
    assert.equal(capabilityToAdmissionChannel("task_completion"), "ch_task_completion");
    assert.equal(capabilityToAdmissionChannel("format_compliance"), "ch_format_compliance");
    assert.equal(capabilityToAdmissionChannel("business_judgment"), "ch_decision_accuracy");
    assert.equal(capabilityToAdmissionChannel("safety_policy"), "ch_uncertainty");
  });
});
