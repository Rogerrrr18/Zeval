...
## 什么时候使用
当 Zeval run `run_1779759984994` 暴露出以下问题时使用本 skill：
...
## 关键 bad case
- 第 3 轮出现失败信号：Some Punjabi kind of foo：severity=0.20，tags=goal_partial，建议=将该片段沉淀到 bad case 池，并纳入下一轮 sample batch 与回放对比。
- 第 4 轮出现失败信号：Found 5 restaurants for ：severity=0.20，tags=goal_partial，建议=将该片段沉淀到 bad case 池，并纳入下一轮 sample batch 与回放对比。
