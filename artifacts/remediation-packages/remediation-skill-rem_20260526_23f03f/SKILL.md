...
## 什么时候使用
当 Zeval run `run_1779755111189` 暴露出以下问题时使用本 skill：
...
## 关键 bad case
- 第 22 轮出现失败信号：Sorry, They don't have l：severity=0.20，tags=goal_partial，建议=将该片段沉淀到 bad case 池，并纳入下一轮 sample batch 与回放对比。
- 第 7 轮出现失败信号：Any other suggestions?：severity=0.18，tags=question_repeat，建议=把策略改为先直接回答问题，再扩展背景，避免用户重复追问。
- 第 5 轮出现失败信号：Is there any other resta：severity=0.18，tags=question_repeat，建议=把策略改为先直接回答问题，再扩展背景，避免用户重复追问。
