    ...
    var Help2 = class {
      ...
      groupItems(unsortedItems, visibleItems, getGroup) {
        ...
        unsortedItems.forEach((item) => {
          ...
          if (!result2.has(group)) result2.set(group, []);
        });
        visibleItems.forEach((item) => {
          const group = getGroup(item);
          if (!result2.has(group)) {
            result2.set(group, []);
          }
          result2.get(group).push(item);
        });
        return result2;
      }
      /**
...
function registerRunsCommand(program3) {
  const runs = program3.command("runs").description("List and inspect saved evaluate runs");
...
program2.name("zeval").description(
...
registerEvaluateCommand(program2);
registerHarvestCommand(program2);
registerPackageCommand(program2);
